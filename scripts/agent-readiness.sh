...
contains() { # contains <url> <pattern> <label>
  if body "$1" | grep -qEi -- "$2"; then ok "$3"; else bad "$3 ($1 ~ $2)"; fi
}
...
FIRST_POST=$(body "$BASE/sitemap.xml" | grep -o '<loc>[^<]*/posts/[^<]\+</loc>' | head -1 | sed -e 's/<[^>]*>//g' -e "s|^https\?://[^/]*|$BASE|")
...
echo "$H" | grep -qE '"@type": *"Person"' && ok "Person entity on home" || bad "no Person JSON-LD on home"
echo "$H" | grep -q 'sameAs' && ok "Person has sameAs links" || bad "Person missing sameAs"
[ -n "$FIRST_POST" ] && contains "$FIRST_POST" '"@type": *"Article"' "Article JSON-LD on posts"

hdr "5. Crawl hygiene"
status_is "$BASE/sitemap.xml" "sitemap.xml exists"
echo "$H" | grep -qE 'rel="?canonical' && ok "canonical link on home" || bad "no canonical on home"
